...
| `assets/프로젝트 이미지 (썸네일).png` | 흰 스튜디오 배경에서 모델이 SKN 앱 화면이 열린 휴대폰을 들고 있으며, 왼쪽에 `skn` 워드마크와 `매일의 경험이, 나만의 기준이 되도록` 문구가 배치된 가로 브랜드 이미지. | 1800×1200 PNG(알파 채널 포함) 보존 원본. 링크 공유 미리보기용으로만 사용하며 제품 UI나 데스크톱 쇼케이스 배경에는 사용하지 않는다. |
