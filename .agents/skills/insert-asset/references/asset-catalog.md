...
| `체크표시 애니메이션.mp4` | 버블 안에서 체크(✓)가 그려지는 모습. | MP4(~2.5MB) | 흰/투명 | 완료·성공 순간의 성공 애니메이션. 웹 파생본은 레티나 모바일 표시를 위해 중앙 구도를 유지한 720×720 `frontend/public/skn-assets/check-motion.{webm,mp4,png}`를 사용한다. |
...
| `제품 성과.svg` | 은은한 블루 실버 메달과 리본, 월계수·별을 결합한 SKN 전용 성과 마크. 특정 외부 어워드 로고를 복제하지 않고 객관적 수상·랭킹 이력을 나타낸다. | SVG, viewBox 48×48 | 투명 | 제품 상세의 출처 확인된 외부 성과 카드. 배포본은 `frontend/public/skn-assets/product-achievement.svg`. |
...
| 파일 | 내용 | 용도 |
| --- | --- | --- |
| `assets/썸네일.png` | 모델이 SKN 앱(온보딩: "당신의 피부를 연구할 준비가 되었어요", 상단에 뒤집힌 N 심벌)을 든 3217×1856 가로 마케팅 이미지. | 보존 원본. 발표와 데스크톱 쇼케이스 파생본의 원본이며 직접 제품 웹에 싣지 않는다. |
| `assets/프로젝트 이미지 (썸네일).png` | 흰 스튜디오 배경에서 모델이 SKN 앱 화면이 열린 휴대폰을 들고 있으며, 왼쪽에 `skn` 워드마크와 `매일의 경험이, 나만의 기준이 되도록` 문구가 배치된 가로 브랜드 이미지. | 1800×1200 PNG(알파 채널 포함) 보존 원본. 저장소 README 히어로와 링크 공유 이미지의 원본으로 사용하며 제품 UI나 데스크톱 쇼케이스 배경에는 사용하지 않는다. |
| `frontend/public/skn-assets/og-cover.jpg` | 위 `프로젝트 이미지 (썸네일).png`를 로고·얼굴·휴대폰이 모두 남도록 1200×630으로 맞춘 JPEG 파생본. | `skn.today`의 Open Graph·Twitter 대형 공유 카드. 캐시 갱신을 위해 메타 URL에 `?v=20260817`을 붙인다. |
