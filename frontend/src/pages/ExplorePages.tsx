...
export function ExplorePage() {
  ...
  return <Screen nav={false}>
    ...
    <div className="px-5 pb-8 pt-4">
      ...
      {!deferredQuery && contextualProduct && <section className="mt-8"><div className="flex items-end justify-between"><div><p className="text-xs font-medium tracking-[.08em] text-[#7892bb]">FOR YOU</p><h2 className="mt-1 text-lg font-medium">{hasPersonalContext ? '내 기록에서 다시 보기' : '먼저 둘러볼 제품'}</h2></div></div><Link to={productPath(contextualProduct.id, returnTo)} className="group mt-4 flex min-h-[150px] overflow-hidden rounded-[26px] border border-[#d9e6ff] bg-[#f8fbff] transition hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(68,96,145,.12)] active:scale-[.99]"><span className="grid w-[140px] shrink-0 place-items-center bg-white/70 p-3"><ProductGlyph category={contextualProduct.category} src={contextualProduct.imageUrl}/></span><span className="flex min-w-0 flex-1 flex-col p-4"><span className="flex min-w-0 items-center gap-1.5 text-[#7892bb]"><BrandIdentity name={contextualProduct.brand} logoUrl={contextualProduct.brandLogoUrl} size="xs"/><span aria-hidden className="text-xs">·</span><span className="truncate text-xs">{contextualProduct.category}</span></span><strong className="mt-2 line-clamp-2 text-lg font-medium leading-6">{contextualProduct.name}</strong><span className="mt-auto inline-flex items-center gap-1 pt-3 text-xs font-medium text-[#5f7396]">{hasPersonalContext ? `내 기록 ${contextualProduct.personalRecordCount}건과 비교` : '제품 정보 살펴보기'}<ArrowRight size={14} className="transition group-hover:translate-x-0.5"/></span></span></Link></section>}
  ...
  return <Screen nav={false} className="bg-white pb-40">
    ...
    <div className="px-5 pb-8 pt-3">
      <section className="overflow-hidden rounded-[24px] border border-[#cfe0ff] bg-[#fbfcff]">
        ...
        <div className="px-5 pb-6 pt-5"><div className="flex min-w-0 items-center gap-1.5 text-[#737880]"><BrandIdentity name={brand} logoUrl={data.brandLogoUrl} size="sm"/><span aria-hidden className="text-xs">·</span><span className="truncate text-xs">{category}</span></div><h1 className="mt-2 text-3xl font-medium leading-[1.18] tracking-[-.05em]">{name}</h1><div className="mt-4 flex flex-wrap gap-2">{data.inCurrentRoutine && <span className="rounded-full bg-[#eef4e9] px-3 py-1.5 text-xs font-medium text-[#657253]">현재 루틴</span>}<span className="rounded-full bg-[#edf3ff] px-3 py-1.5 text-xs font-medium text-[#5f7396]">경험 {data.personalRecordCount}건</span></div></div>
      </section>
      ...
      <dl className="mt-6 divide-y divide-line border-y border-line"><div className="grid grid-cols-[92px_1fr] gap-3 py-4"><dt className="text-xs text-muted">브랜드</dt><dd><BrandIdentity name={brand} logoUrl={data.brandLogoUrl} size="sm"/></dd></div><div className="grid grid-cols-[92px_1fr] gap-3 py-4"><dt className="text-xs text-muted">제품 유형</dt><dd className="text-sm font-medium">{category}</dd></div><div className="grid grid-cols-[92px_1fr] gap-3 py-4"><dt className="text-xs text-muted">등록일</dt><dd className="text-sm font-medium">{formatProductDate(data.addedAt)}</dd></div></dl>
    </div>
...
function ProductHero({ product }: { product: Product }) {
  ...
  return <section>
    ...
    <div className="px-[30px] pb-6 pt-5">
      <div className="flex min-w-0 items-center gap-2 text-[#777]"><BrandIdentity name={product.brand} logoUrl={product.brandLogoUrl} size="sm"/><span className="truncate text-xs">{product.category}</span></div>
...
function ShelfCard({ item, onStart }: { item: UserProduct; onStart: () => void }) {
  ...
  return <button type="button" onClick={onStart} aria-label={`${name} 상세 보기`} className="group flex min-h-[242px] w-full flex-col overflow-hidden rounded-[24px] border border-black/[.055] bg-white text-left shadow-[0_6px_22px_rgba(45,58,77,.055)] transition hover:-translate-y-1 hover:shadow-[0_14px_30px_rgba(45,58,77,.12)] active:scale-[.99]">
    ...
    <span className="flex min-h-[94px] flex-col p-3.5"><span className="flex min-w-0 items-center gap-1 text-black/42"><BrandIdentity name={product?.brand || item.customBrand || '브랜드 미입력'} logoUrl={item.brandLogoUrl} size="xs"/><span aria-hidden className="text-xs">·</span><span className="truncate text-xs">{category}</span></span><strong className="mt-1.5 line-clamp-2 text-[15px] font-medium leading-5 tracking-[-.02em]">{name}</strong><span className="mt-auto flex items-center justify-end pt-2 text-black/45"><ArrowRight className="transition group-hover:translate-x-0.5" size={15}/></span></span>
  </button>
}
