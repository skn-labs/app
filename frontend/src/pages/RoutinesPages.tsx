...
export function RoutineListPage() {
  ...
  const cards: RoutineCarouselEntry[] = [
    ...routines.map((routine, index) => ({ kind: 'routine' as const, key: `routine-${routine.id}`, routine, image: routineCards[index % routineCards.length] })),
    ...Array.from({ length: Math.max(1, 3 - routines.length) }, (_, index) => ({
  ...
  return <Screen className="bg-white">
    ...
    <section className="mt-10" aria-label="나의 루틴 카드">
      <div ref={carousel} onScroll={event => handleScroll(event.currentTarget)} className="hide-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto px-[calc((100%_-_260px)/2)] pb-5">
        {cards.map(card => card.kind === 'routine'
          ? <RoutineCarouselCard key={card.key} routine={card.routine} current={current.data} image={card.image} expanded={expandedId === card.routine.id} onExpand={() => setExpandedId(card.routine.id)} onCollapse={() => setExpandedId(null)}/>
          : <CreateRoutineCard key={card.key} image={card.image} description={card.description}/>)}
      </div>
...
function CreateRoutineCard({ image, description }: { image: string; description: string }) {
  return <Link data-routine-card to="/routine/edit" aria-label="새 루틴 만들기" className="group relative h-[360px] w-[260px] shrink-0 snap-center overflow-hidden rounded-[22px] text-left shadow-[0_8px_26px_rgba(0,0,0,.10)] transition hover:-translate-y-0.5 active:scale-[.99]">
