  ...
  return <Screen nav={false} className="bg-[#fbfcff]">
    ...
    <div className="px-5 pb-12 pt-2">
      ...
      {view === 'PROFILE' ? <>
        ...
        <p className="mt-2 text-[11px] leading-5 text-[#798493]">직접 남긴 느낌을 당시 제품과 루틴에 연결해 보관해요.</p>
...
function ExperienceMap({ patterns, recordCount, loading }: { patterns: Pattern[]; recordCount: number; loading: boolean }) {
  ...
  const emptyCopy = recordCount === 0 ? '내 경험에 맞춰 여섯 축이 달라집니다.' : recordCount === 1 ? '비슷한 조건의 경험이 하나 더 생기면 비교를 시작해요.' : '기록은 있지만 조건과 느낌이 비슷한 경험이 아직 없어요.'
