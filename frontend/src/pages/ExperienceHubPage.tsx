  ...
  return <Screen nav={false} className="bg-white">
    ...
    <div className="px-5 pb-8 pt-3">
      ...
      <section className="mt-9" aria-labelledby="active-experience-title">
        <p className="text-xs font-medium text-[#5f7396]">7일 동안 사용감을 남기는 조합</p>
        <h2 id="active-experience-title" className="mt-1.5 text-xl font-medium tracking-[-.035em]">확인 중인 경험</h2>
        {experience
      ...
      <section className="mt-10" aria-labelledby="baseline-routine-title">
        ...
        {baseline.data
          ...
          : <Card className="mt-3 border-dashed"><p className="text-sm font-medium">아직 비교 기준이 없어요</p><p className="mt-1 text-xs leading-5 text-[#73766f]">사용 경험에서 불편함이 없었다고 직접 남긴 루틴만 이후 변경 비교의 기준이 됩니다.</p></Card>}
      </section>
...
function RoutineSummary({ routine, baseline = false }: { routine: Routine; baseline?: boolean }) {
  ...
  return <Card className={`mt-3 overflow-hidden p-0 ${baseline ? 'bg-[#f7f8f5]' : ''}`}>
    <Link to={`/routines/${routine.id}`} className="flex min-h-[68px] items-center justify-between px-4 py-4"><div><span className="text-xs font-medium text-[#73766f]">{baseline ? '불편함이 없었다고 남긴 조합' : '실제 사용 중인 조합'}</span><h3 className="mt-1 text-sm font-medium">{routine.name}</h3></div><Clock3 size={17} className="text-[#73766f]"/></Link>
    {groups.map(group => <div key={group.label} className="border-t border-[#e7e9e3]"><div className="bg-white/60 px-4 py-2 text-xs font-medium text-[#73766f]">{group.label} · {group.items.length}개</div>{group.items.map((item, index) => <div key={`${group.label}-${item.userProductId}`} className="flex items-center gap-3 border-t border-[#e7e9e3]/70 px-4 py-3"><span className="grid size-6 place-items-center rounded-full bg-white text-xs font-medium">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-medium">{item.productName}</p><p className="mt-0.5 text-xs text-[#73766f]">{item.category} · {item.frequency}</p></div></div>)}</div>)}
    {baseline && <p className="border-t border-[#e7e9e3] px-4 py-3 text-xs leading-5 text-[#73766f]">적합성이나 안전 판정이 아니라, 이후 변경을 비교하기 위한 내 기록입니다.</p>}
  </Card>
}
