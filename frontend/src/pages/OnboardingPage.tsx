  ...
  return <PrototypePhone>
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-7">
      <div className="flex items-end justify-between pt-5">
        ...
        <span className="text-[11px] font-semibold tabular-nums text-[#8e8e93]">{step + 1} / {STEPS.length}</span>
      </div>
      ...
      <div key={current.key} className={`hide-scrollbar mt-7 min-h-0 flex-1 overflow-y-auto pb-4 ${direction === 'forward' ? 'animate-onboard-forward' : 'animate-onboard-back'}`}>
      ...
      <div className="flex shrink-0 gap-2 pb-2 pt-1">
        <button type="button" onClick={() => move(Math.max(0, step - 1), 'back')} disabled={step === 0 || complete.isPending} className="h-[52px] w-[104px] shrink-0 rounded-full bg-[#f2f2f7] text-[15px] font-semibold transition active:scale-[.98] disabled:text-[#c7c7cc] disabled:active:scale-100">이전</button>
        <button type="button" disabled={!canProceed || complete.isPending} onClick={() => step === STEPS.length - 1 ? complete.mutate() : move(step + 1, 'forward')} className="h-[52px] flex-1 rounded-full bg-[#0a0a0a] text-[15px] font-semibold text-white shadow-[0_8px_20px_rgba(0,0,0,.12)] transition active:scale-[.98] disabled:bg-[#e5e5ea] disabled:text-[#c7c7cc] disabled:shadow-none disabled:active:scale-100">{complete.isPending ? '프로필 저장 중…' : step === STEPS.length - 1 ? '프로필 완성하기' : '다음'}</button>
      </div>
    </div>
...
function ProgressDashes({ current, total }: { current: number; total: number }) {
  return <div role="progressbar" aria-valuemin={1} aria-valuemax={total} aria-valuenow={current + 1} aria-label={`${total}단계 중 ${current + 1}단계`} className="flex min-w-0 flex-1 shrink-0 gap-1.5 pr-4">
    {Array.from({ length: total }, (_, index) => <i key={index} className={`h-[3px] min-w-0 flex-1 rounded-full transition-colors duration-300 ${index <= current ? 'bg-[#0a0a0a]' : 'bg-[#e5e5ea]'}`}/>) }
  </div>
}
...
function AgeWheel({ value, onChange }: { value: DraftProfile['ageRange']; onChange: (value: SkinProfile['ageRange']) => void }) {
  ...
  return <div><p className="mb-2 text-center text-[11px] text-[#c7c7cc]">위아래로 움직이거나 나이대를 눌러주세요</p><div className="relative h-[280px]">
    ...
    <div ref={list} onScroll={handleScroll} role="radiogroup" aria-label="연령대" className="hide-scrollbar relative h-full snap-y snap-mandatory overflow-y-auto py-[112px]">
      {AGE_RANGES.map(([code, label]) => <button type="button" role="radio" key={code} onClick={() => onChange(code)} aria-checked={value === code} className={`block h-14 w-full snap-center bg-transparent text-center transition ${value === code ? 'text-[26px] font-bold text-[#0a0a0a]' : 'text-[22px] font-medium text-[#c7c7cc]'}`}>{label}</button>)}
    </div>
...
function OptionList<T extends string>({ options, value, onChange }: { options: readonly (readonly [T, string])[]; value: T | null; onChange: (value: T) => void }) {
  return <div role="radiogroup" className="flex flex-col gap-2.5">{options.map(([code, label]) => <button type="button" role="radio" aria-checked={value === code} key={code} onClick={() => onChange(code)} className={`flex h-[54px] w-full items-center justify-between rounded-[14px] border px-5 text-left text-[15px] transition active:scale-[.99] ${value === code ? 'border-[#0a0a0a] bg-[#0a0a0a] font-semibold text-white shadow-[0_6px_16px_rgba(0,0,0,.1)]' : 'border-[#e5e5ea] bg-[#f2f2f7] text-[#0a0a0a]'}`}><span>{label}</span><span aria-hidden="true" className={`grid size-5 place-items-center rounded-full border text-[11px] ${value === code ? 'border-white bg-white text-black' : 'border-[#c7c7cc] text-transparent'}`}>✓</span></button>)}</div>
}

function ConditionScale({ value, onChange }: { value: number | null; onChange: (value: number) => void }) {
  return <div className="pt-4"><div className="mb-3 flex justify-between text-xs text-[#8e8e93]"><span>매우 불안정</span><span>보통</span><span>매우 안정</span></div><div role="radiogroup" aria-label="현재 피부 상태" className="relative flex justify-between before:absolute before:left-5 before:right-5 before:top-1/2 before:h-px before:-translate-y-1/2 before:bg-[#e5e5ea]">{[1, 2, 3, 4, 5].map(number => <button type="button" role="radio" aria-checked={value === number} aria-label={`${number}점, ${CONDITION_LABELS[number]}`} key={number} onClick={() => onChange(number)} className={`relative size-11 rounded-full border text-[15px] transition active:scale-95 ${value === number ? 'scale-110 border-[#0a0a0a] bg-[#0a0a0a] font-semibold text-white shadow-[0_5px_14px_rgba(0,0,0,.16)]' : 'border-[#e5e5ea] bg-white text-[#8e8e93]'}`}>{number}</button>)}</div><div aria-live="polite" className={`mt-7 rounded-[14px] px-4 py-3 text-center text-[13px] transition ${value ? 'bg-[#f2f2f7] text-[#3a3a3c]' : 'bg-transparent text-[#c7c7cc]'}`}>{value ? CONDITION_LABELS[value] : '가장 가까운 상태를 골라주세요'}</div></div>
}
...
function ChipGroups({ groups, selected, onToggle }: { groups: readonly { title: string; options: readonly ChipOption[] }[]; selected: string[]; onToggle: (value: string) => void }) {
  return <div className="flex flex-col gap-6">{groups.map(group => {
    ...
    return <section key={group.title}><div className="mb-2.5 flex items-center justify-between"><h2 className="text-[13px] font-semibold text-[#8e8e93]">{group.title}</h2><span className="text-[10px] text-[#c7c7cc]">{groupCount > 0 ? `${groupCount}개 선택` : ''}</span></div><div className="flex flex-wrap gap-2">{group.options.map(option => {
      const value = chipValue(option)
      const selectedOption = selected.includes(value)
      return <button type="button" aria-pressed={selectedOption} key={`${group.title}-${value}`} onClick={() => onToggle(value)} className={`min-h-11 rounded-full border px-4 py-2 text-sm transition active:scale-95 ${selectedOption ? 'border-[#0a0a0a] bg-[#0a0a0a] font-semibold text-white shadow-[0_4px_12px_rgba(0,0,0,.1)]' : 'border-[#e5e5ea] bg-white text-[#0a0a0a]'}`}>{selectedOption && <span aria-hidden="true" className="mr-1.5">✓</span>}{chipLabel(option)}</button>
    })}</div></section>
  })}</div>
}
