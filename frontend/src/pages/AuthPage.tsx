...
export function AuthPage() {
  ...
  if (screen === 'welcome') return <PrototypePhone>
    ...
    <div className="safe-bottom flex min-h-0 flex-1 flex-col px-7">
      <div className="pt-10"><h1 className="text-[24px] font-medium leading-[1.35] tracking-[-.035em]">당신의 피부를 연구할<br/>준비가 되었어요.</h1><p className="mt-3 text-[13px] leading-[1.65] text-[#73766f]">몇 가지 질문으로 당신에게 맞는<br/>케어를 시작할게요.</p></div>
      ...
      <button type="button" onClick={() => moveTo('login')} className="h-[52px] w-full rounded-full bg-[#0a0a0a] text-[15px] font-medium text-white shadow-[0_8px_22px_rgba(0,0,0,.14)] transition active:scale-[.98]">로그인</button>
  ...
  return <PrototypePhone>
    ...
    <form onSubmit={event => { event.preventDefault(); auth.mutate() }} className="safe-bottom flex min-h-0 flex-1 flex-col px-7 pt-10">
      ...
      <div className="mt-9 flex flex-col gap-4">
        <label><span className="mb-1.5 block text-[13px] font-medium text-[#73766f]">아이디</span><input autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={username} onChange={event => { setUsername(event.target.value.toLowerCase().trim()); auth.reset() }} placeholder="skn_user" className="field-control h-[52px] w-full px-4 text-[15px]"/></label>
        <label><span className="mb-1.5 block text-[13px] font-medium text-[#73766f]">비밀번호</span><div className="field-control relative"><input autoComplete={signup ? 'new-password' : 'current-password'} type={showPassword ? 'text' : 'password'} value={password} maxLength={72} onChange={event => { setPassword(event.target.value); auth.reset() }} placeholder="8자 이상" className="h-[52px] w-full bg-transparent px-4 pr-12 text-[15px] outline-none"/><button type="button" aria-label={showPassword ? '비밀번호 숨기기' : '비밀번호 보기'} onClick={() => setShowPassword(value => !value)} className="absolute right-1 top-1 grid size-[44px] place-items-center rounded-full text-[#73766f] transition hover:bg-white">{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div></label>
