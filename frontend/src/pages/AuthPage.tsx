...
export function AuthPage() {
  ...
  if (screen === 'welcome') return <PrototypePhone>
    ...
    <div className="flex min-h-0 flex-1 flex-col px-7">
      ...
      <div className="flex flex-1 items-center justify-center"><img src="/skn-assets/onboarding-orb.png" alt="투명한 세럼 구체" className="size-[230px] object-contain"/></div>
      <button type="button" onClick={() => moveTo('login')} className="h-[52px] w-full rounded-full bg-[#0a0a0a] text-[15px] font-semibold text-white">로그인</button>
  ...
  return <PrototypePhone>
    ...
    <form onSubmit={event => { event.preventDefault(); auth.mutate() }} className="flex min-h-0 flex-1 flex-col px-7 pt-10">
      ...
      <div className="mt-9 flex flex-col gap-4">
        <label><span className="mb-1.5 block text-[13px] font-medium text-[#8e8e93]">아이디</span><input autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={username} onChange={event => setUsername(event.target.value.toLowerCase().trim())} placeholder="skn_user" className="h-[50px] w-full rounded-xl border border-transparent bg-[#f2f2f7] px-4 text-[15px] outline-none placeholder:text-[#c7c7cc] focus:border-[#0a0a0a]"/></label>
        <label><span className="mb-1.5 block text-[13px] font-medium text-[#8e8e93]">비밀번호</span><div className="relative"><input autoComplete={signup ? 'new-password' : 'current-password'} type={showPassword ? 'text' : 'password'} value={password} maxLength={72} onChange={event => setPassword(event.target.value)} placeholder="8자 이상" className="h-[50px] w-full rounded-xl border border-transparent bg-[#f2f2f7] px-4 pr-12 text-[15px] outline-none placeholder:text-[#c7c7cc] focus:border-[#0a0a0a]"/><button type="button" aria-label={showPassword ? '비밀번호 숨기기' : '비밀번호 보기'} onClick={() => setShowPassword(value => !value)} className="absolute right-1 top-1 grid size-[42px] place-items-center text-[#8e8e93]">{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div></label>
      ...
      <button type="submit" disabled={auth.isPending || !username || !password} className="h-[52px] w-full rounded-full bg-[#0a0a0a] text-[15px] font-semibold text-white disabled:bg-[#e5e5ea] disabled:text-[#c7c7cc]">{signup ? '회원가입' : '로그인'}</button>
